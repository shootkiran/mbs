...
class RegistrationController extends Controller
{
    ...
    public function storeOneRegistration($collection_id, $name, $dobad, $level, $user_id = null, $contact = null, $dummy = 0)
    {
        ...
        $now = Carbon::now();
        $empty_row = DB::table($level_table)->where('examination_id', $examination->id)->where('reg_id', 0)->orderBy('sn','Desc')->first();
        ...
        if ($empty_row) {
            $empty_row_id = $empty_row->id;
            DB::table($level_table)->where('id', $empty_row_id)->update([
                'reg_id' => $reg->id
            ]);
        } else {
